...
name: lint
description: >-
  Automatically corrects Ruby and Rails code style using RuboCop and formatting
  tools. Use when fixing lint errors, standardizing code style, or when
  encountering RuboCop violations, code formatting issues, or style problems.
...
```bash
bundle exec rubocop        # Check entire project
...
```bash
bundle exec rubocop -a app/models/       # Format all models
bundle exec rubocop -a app/controllers/  # Format all controllers
```
