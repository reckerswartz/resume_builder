...
RSpec.describe 'Templates', type: :request do
  ...
  describe 'GET /templates' do
    it 'renders the signed-in template marketplace with only user-visible templates' do
      ...
      expect(response.body).to include('Search first, then open the strongest few')
  ...
  describe 'GET /templates/:id' do
    it 'renders the template detail page with the shared sample renderer' do
      ...
      expect(response.body).to include('Back to templates')
      ...
      expect(response.body).to include('You can change this in the builder anytime.')
      expect(response.body).to include('Balanced layouts work well for broad experience. Sidebar layouts group secondary sections.')
      expect(response.body).not_to include('Treat this as the starting visual cue for the builder, not a locked brand decision.')
      expect(response.body).not_to include('Balanced layouts suit broad experience storytelling, while sidebar-heavy layouts keep secondary sections grouped and quieter.')
