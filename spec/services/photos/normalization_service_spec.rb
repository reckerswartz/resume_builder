...
RSpec.describe Photos::NormalizationService, type: :service do
  def create_source_photo_asset(photo_profile:, filename: 'source-headshot.png', status: :uploaded, metadata: {})
    PhotoAsset.new(photo_profile:, asset_kind: :source, status:, metadata: metadata).tap do |photo_asset|
  ...
  describe '#call' do
    it 'creates a normalized asset and records vips metadata when image processing succeeds' do
      source_asset = create_source_photo_asset(photo_profile:, metadata: { 'width' => 320, 'height' => 240 })
      stub_vips_processing(width: 800, height: 1000)
      result = nil
      ...
      expect(result.asset).to be_normalized
      ...
      expect(result.metadata['normalized_at']).to be_present
      expect(source_asset.reload).to be_ready
    end

    it 'falls back to a passthrough copy when image processing is unavailable' do
    ...
    it 'marks the source asset failed and returns the error when normalized asset persistence fails' do
      ...
      expect(source_asset.reload).to be_failed
    end
  end
end
