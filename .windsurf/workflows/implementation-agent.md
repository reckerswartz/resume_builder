...
GH-2. If not already on a workflow branch (from an upstream `/tdd-red-agent` or `/feature-plan` invocation), **create a GitHub issue** with structured context:
    ```bash
