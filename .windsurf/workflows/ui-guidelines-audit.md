...
GH-2. **Create a GitHub issue** with full structured context for the finding being fixed:
    ...
    bin/gh-bridge/create-issue \
      ...
      --description "<clear description of the guideline violation>" \
      --expected "<expected UI guideline-compliant behavior>" \
      --actual "<actual non-compliant behavior>" \
      ...
      --logs "<guideline scores, compliance details>" \
      --registry-path "docs/ui_audits/guidelines_review/registry.yml" \
...
GH-6. **Determine next task** after completion:
    ```bash
    // turbo
    bin/gh-bridge/next-task --workflow ui-guidelines-audit
    ```
