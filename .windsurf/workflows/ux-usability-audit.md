...
6. **Browser session isolation**: follow `.windsurf/workflows/playwright-session-guide.md` for all Playwright interactions. Each audit run must use its own isolated browser context — never reuse sessions from prior workflow runs. Tag the session with `workflowId: ux-usability-audit` and the target `issueId` (e.g. page key or issue key). Close the session after the audit batch completes.
...
GIT-2. **After validation passes** (Phase 4), stage, commit, and push:
    ```bash
    git add -A
    git commit -m "ux-usability-audit: <description of the fix>"
